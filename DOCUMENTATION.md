...
- **How data is judged/categorized:**
  1. **LLM Extraction (`BotService` -> `PlantSetupStore`):** The Telegram bot pipes the user message directly to Gemini to extract structured `plants` and `rooms` JSON data. The prompt rigorously prevents Gemini from assuming plant placement or variety details unless explicitly described.
  2. **Deterministic Fallback (`MemoryExtractor`):** The local CLI orchestrator pathway relies on simple keyword rules parsing user text for explicit event tagging (like watering, fertilizing).
- **What is saved:** 
  - Room profiles including name, type, a list of windows (up to 3), size, and grow light flags. City data acts as a home-level property mostly placed onto the room.
  - Plant profiles including name, species, soil type, and fertilizer type. They are linked to `room_id`s to establish a clean Home -> Room -> Plant hierarchy.
- **How data is accessed:** 
  - LLM Context: When the bot fields a question (e.g. "describe my room"), the `PlantSetupStore.build_user_setup_summary(user_id)` generates a rich, readable summary block. This block is injected at the top of the LLM prompt, allowing "My Plants" to actively leverage saved properties during conversations without hitting the database again inside the response engine.
  - Programmatic Context: For CLI usage and analysis logic, `ContextBuilder.build` parses the files to pass full dictionary payloads into decision engines.
